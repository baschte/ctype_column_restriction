...
namespace Baschte\CtypeColumnRestriction\View;
...
class PageLayoutView extends \TYPO3\CMS\Backend\View\PageLayoutView
{
    ...
    public function getTable_tt_content($id)
    {
        ...
        foreach ($langListArr as $lP) {
            ...
            foreach ($cList as $columnId) {
                ...
                //get page be-layout and allowed ctypes

                $pageTSConfig = BackendUtility::getPagesTSconfig($id);
                $backendLayout = $this->getBackendLayoutFromPageUid($id);
                $backendLayoutConfigArray = $pageTSConfig['mod.']['web_layout.']['BackendLayouts.'][$backendLayout . '.']['config.']['backend_layout.']['rows.'];
                $allowedCTypes = implode(",", $this->getAllowedCTypesForColPos($columnId, $backendLayoutConfigArray));

                if ($this->getBackendUser()->checkLanguageAccess($lP)) {
                    $content[$columnId] .= '
                    <div class="t3-page-ce t3js-page-ce" data-allowed="' . $allowedCTypes . '" data-page="' . (int)$id . '" id="' . StringUtility::getUniqueId() . '">
                    ...
                    foreach ((array)$rowArr as $rKey => $row) {
                        ...
                        if (is_array($row) && !VersionState::cast($row['t3ver_state'])->equals(VersionState::DELETE_PLACEHOLDER)) {
                            ...
                            if (!$disableMoveAndNewButtons
                                ...
                                $singleElementHTML .= '<a href="' . htmlspecialchars($url) . '" title="'
                                    . htmlspecialchars($this->getLanguageService()->getLL('newContentElement')) . '" class="btn btn-default btn-sm">'
                                    . $this->iconFactory->getIcon('actions-document-new', Icon::SIZE_SMALL)->render()
                                    . ' '
                                    . htmlspecialchars($this->getLanguageService()->getLL('content')) . '</a>';
    ...
    private function getBackendLayoutFromPageUid($pageUid)
    {
        ...
        // else check one level up
            ...
            $pageRootLine = $pageRepository->getRootLine($pageUid);
     ...
     * @return array
     */
    private function getAllowedCTypesForColPos($colPos, $backendLayoutConfigArray) {
        // loop through backend_layout definitions and found col with given colpos
        foreach ($backendLayoutConfigArray as $backendLayoutRow) {
            if (is_array($backendLayoutRow['columns.']) && !empty($backendLayoutRow['columns.'])) {
                foreach ($backendLayoutRow['columns.'] as $backendLayoutColumns) {
                    ...
                    if(isset($backendLayoutColumns['colPos']) && $backendLayoutColumns['colPos'] !== '' && intval($colPos) === intval($backendLayoutColumns['colPos'])) {
                        ...
                        if(isset($backendLayoutColumns['allowed']) && $backendLayoutColumns['allowed'] !== '') {
                            return GeneralUtility::trimExplode(',', $backendLayoutColumns['allowed']);
                        } else {
                            return ['*'];
                        }
